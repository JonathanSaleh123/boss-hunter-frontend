...
import { GameOverModal } from './arena/GameOverModal';
...
const ArenaUI = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black p-6 overflow-hidden relative">
      ...
      {/* The top-level buttons and timer have been removed from here */}
      ...
      {/* Main content area */}
      <div className={`flex justify-center gap-8 mb-8 relative z-10 mt-0 transition-filter duration-500 ${isGameOver ? 'blur-md' : ''}`}>
        
        {/* Boss Display (No changes) */}
        <div className={`relative glass-card rounded-3xl p-6 border-2 transition-all duration-500 shadow-2xl self-start ${ boss.isEnraged ? 'border-red-500 shadow-red-500/30 animate-pulse' : 'border-purple-500 shadow-purple-500/20' }`}>
            <div className="flex flex-col items-center gap-4">
                ...
                <div className={`w-40 h-40 rounded-2xl overflow-hidden border-4 transition-all duration-300 ${ boss.isEnraged ? 'border-red-500' : 'border-purple-500' }`}>
                    <img src={boss.imageUrl} alt={boss.name} className={`w-full h-full object-cover transition-all duration-300 ${ boss.isEnraged ? 'filter brightness-110 contrast-110' : '' }`} onError={(e) => { (e.currentTarget as HTMLImageElement).src = 'https://placehold.co/400x400/1e293b/ffffff?text=Boss'; }} />
                </div>
        ...
        {/* --- NEW: Wrapper for Chat and Controls --- */}
        <div className="flex gap-8">
            {/* Battle Chat (No changes) */}
            <div className="w-[600px] h-[400px] glass-card rounded-xl border-2 border-slate-600 shadow-2xl">
            ...
            <div className="w-56 flex flex-col gap-4">
                <div className="glass-card rounded-xl border-2 border-slate-600 p-4 shadow-2xl">
                    <GameStatusDisplay gameState={gameState} timer={timer} onStartGame={handleStartGame} />
                </div>
                <button 
                    onClick={handleToggleMusic}
                    className="w-full flex items-center justify-center gap-2 bg-cyan-800/50 hover:bg-cyan-700/70 text-cyan-300 hover:text-white transition-all text-sm font-bold px-4 py-2 rounded-lg"
                >
                    <Music size={16} />
                ...
                <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-red-800/50 hover:bg-red-700/70 text-red-300 hover:text-white transition-all text-sm font-bold px-4 py-2 rounded-lg"
                >
                    <LogOut size={16} />
                    Leave Room
                </button>
      ...
      <div className={`flex justify-center items-end gap-16 relative z-10 transition-filter duration-500 ${isGameOver ? 'blur-md' : ''}`}>
        {players.map((player, index) => (
            <PlayerCard key={player.id} player={player} onShowDetails={setDetailsCharacter} playerIndex={index} />
        ))}
      </div>
      <style jsx>{`.cyber-grid { background-image: linear-gradient(rgba(0, 255, 255, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(0, 255, 255, 0.1) 1px, transparent 1px); background-size: 50px 50px; } .glass-card { background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1); } .boss-float { animation: float 6s ease-in-out infinite; } @keyframes float { 0% { transform: translateY(0px); } 50% { transform: translateY(-10px); } 100% { transform: translateY(0px); } }`}</style>
    </div>
  );
};
