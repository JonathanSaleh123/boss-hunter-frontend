...
import { Sword, Shield, Heart, Skull, Crown, User, Timer as TimerIcon, Check, Play } from 'lucide-react';
...
const ArenaUI = () => {
  ...
  // --- MODIFIED: Renamed ref for clarity ---
  ...
  // --- MODIFIED: Switched to a more reliable scrolling method ---
  useEffect(() => {
  ...
  const PlayerCard = ({ player }: { player: Character }) => {
  ...
  const GameStatus = () => {
    ...
    switch (gameState) {
        case 'WAITING_FOR_ACTIONS':
            ...
            color = 'text-cyan-400';
            break;
        case 'PLAYERS_ATTACKING':
            statusText = 'PARTY ATTACKING';
            color = 'text-orange-400 animate-pulse';
            break;
        case 'BOSS_ATTACKING':
            statusText = 'BOSS ATTACKING';
    ...
    return (
      <div className="glass-card border-slate-600 border-2 rounded-xl px-6 py-3 flex flex-col items-center shadow-lg">
          ...
          {gameState === 'WAITING_FOR_ACTIONS' && (
              <div className="flex items-center gap-2 mt-2 text-2xl text-white font-mono">
  ...
  const StartGameButton = () => (
    <div className="flex flex-col items-center gap-4">
        ...
        <button 
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-cyan-500 text-white font-bold text-lg px-8 py-4 rounded-lg shadow-lg hover:scale-105 transition-transform duration-300"
        >
            <Play />
            START BATTLE
        </button>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black p-6 overflow-hidden relative">
      ...
      <div className="flex justify-center gap-8 mb-8 relative z-10 mt-24">
        ...
        <div className="w-[500px] h-[380px] glass-card rounded-xl border-2 border-slate-600 shadow-2xl">
          <div className="h-full flex flex-col">
            ...
            {/* --- MODIFIED: Added ref to this div, removed the inner placeholder div --- */}
            ...
            <div className="border-t border-slate-600 p-3">
              <form className="flex gap-2" onSubmit={handleSendMessage}>
                <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={isInputDisabled ? "Waiting..." : "Describe your action..."} className="flex-1 bg-slate-800 text-white rounded-lg px-3 py-2 text-sm border border-slate-600 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/20 disabled:bg-slate-900 disabled:cursor-not-allowed" maxLength={200} disabled={isInputDisabled}/>
                <button type="submit" className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all duration-300 hover:scale-105 disabled:bg-slate-700 disabled:from-slate-600 disabled:scale-100 disabled:cursor-not-allowed flex items-center gap-2" disabled={isInputDisabled}>
                    {hasSubmittedAction ? <><Check className="w-4 h-4" /> Locked In</> : "Send Action"}
                </button>
              </form>
            </div>
          </div>
      ...
      <div className="flex justify-center items-end gap-16 relative z-10">
        ...
        {players.length === 1 && gameState === 'IDLE' && (
             <div className="flex flex-col items-center mx-8">
                <StartGameButton />
             </div>
        )}
